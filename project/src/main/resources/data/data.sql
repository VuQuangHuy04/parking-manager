...
-- 3. Bảng Parking Slots (Đã sửa lỗi cú pháp)
...
CREATE TABLE bookings (
    ...
    slot_id INTEGER NOT NULL, -- Thay đổi kiểu dữ liệu thành INTEGER để khớp với id của parking_slots
    start_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    end_time DATETIME,
    total_price REAL DEFAULT 0,
    status TEXT DEFAULT 'ACTIVE' CHECK(status IN ('ACTIVE','DONE','CANCELLED')),
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (slot_id) REFERENCES parking_slots(id) -- Tham chiếu vào ID của slot
);
